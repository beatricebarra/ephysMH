function [paramEsts, negloglik] = Trunc_Norm_Diff_Fit(x,K)
...
elseif K == 2
    start = [.5, Y', std(x(cInd==I(1))), std(x(cInd==I(2)))]; % for 2
elseif K == 3
    start = [1/3, 1/3, Y', std(x(cInd==I(1))), std(x(cInd==I(2))), std(x(cInd==I(3)))]; % if K  3
elseif K == 4
    start = [1/4, 1/4, 1/4, Y', std(x(cInd==I(1))), std(x(cInd==I(2))), std(x(cInd==I(3))), std(x(cInd==I(4)))]; % if K  4
end
...
elseif K == 2
    ...
    pdf_normmixture = @(x,p,mu1,mu2,sigma1,sigma2) ...
        ...
        min((p*pdf_truncnorm(x,mu1,sigma1) - ...
        (1-p)*pdf_truncnorm(x,mu2,sigma2)))+.001) / ...
        trapz((p*pdf_truncnorm(x,mu1,sigma1) - ...
        (1-p)*pdf_truncnorm(x,mu2,sigma2)) - ...
        min((p*pdf_truncnorm(x,mu1,sigma1) - ...
        (1-p)*pdf_truncnorm(x,mu2,sigma2)))+.001);
