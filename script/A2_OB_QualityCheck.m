...
savefigurename = fullfile(folder, 'OB_qualitycheck.png');
...
mywindow = 0.3; 
FR_table = cell(length(OB_units_idx), 4*8 );
baseline_FR_table = cell(length(OB_units_idx), 4*8 );
for i = 1: length(OB_units_idx)
    for j = 1 : 8
        FR_table{i,j} = []; 
